...
// 		while (!exit_check(cmd->next->tok->next) && cmd->next)
// 		{
// 			cmd = cmd->next;
// 			printf("-----------------------------------------\n");
// 			printf("cmd node%d\n", i++);
...
// 				if (!ft_strncmp(cmd->tok->str, "cd", 2))
...
// // gcc -lreadline -fsanitize=address builtin/cd.c libft.a parsing/chunk.c builtin/exit.c free.c init_env_list.c parsing/init.c parsing/tokenizer.c parsing/iterator.c -o hyeslim
// /*
